...
use aya_ebpf::{
    bindings::{BPF_F_RDONLY, BPF_RB_FORCE_WAKEUP, xdp_action::XDP_PASS},
    macros::{map, xdp},
...
const DO_NOT_DELIVER_PACKETS_TO_USER: u8 = 0; // so not send packet to userspace
const DELIVER_PACKETS_TO_USER_VIA_PERF: u8 = 1;
const DELIVER_PACKETS_TO_USER_VIA_RING_WITH_DELAY: u8 = 2;
const DELIVER_PACKETS_TO_USER_VIA_RING_WITH_EPOLL: u8 = 3;

const RING_SIZE: u32 = 1 << 26; // 64 MB
const BENCH_PACKET_LEN: usize = 165; // eth(14) + ipv4(20) + udp(8) + payload(123)
...
pub fn poc_none(ctx: XdpContext) -> u32 {
    process::<DO_NOT_DELIVER_PACKETS_TO_USER>(&ctx)
}

#[xdp]
pub fn poc_perf(ctx: XdpContext) -> u32 {
    process::<DELIVER_PACKETS_TO_USER_VIA_PERF>(&ctx)
}

#[xdp]
pub fn poc_ring_with_delay(ctx: XdpContext) -> u32 {
    process::<DELIVER_PACKETS_TO_USER_VIA_RING_WITH_DELAY>(&ctx)
}

#[xdp]
pub fn poc_ring_with_epoll(ctx: XdpContext) -> u32 {
    process::<DELIVER_PACKETS_TO_USER_VIA_RING_WITH_EPOLL>(&ctx)
}
...
fn process<const MODE: u8>(ctx: &XdpContext) -> u32 {
    ...
    if len != BENCH_PACKET_LEN {
        return XDP_PASS; // Skip packets that were not generated by benchmark
    }
    ...
    match MODE {
        DELIVER_PACKETS_TO_USER_VIA_PERF => {
            PERF.output(ctx, &(PerfEvent { time, len }), len as u32)
        }
        DELIVER_PACKETS_TO_USER_VIA_RING_WITH_DELAY
        | DELIVER_PACKETS_TO_USER_VIA_RING_WITH_EPOLL => {
