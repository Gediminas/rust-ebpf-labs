...
const PACKET_RING_SIZE: u32 = 16;
pub const RX_QUEUE_SIZE: u32 = 8; // 1 << 14
const TX_QUEUE_SIZE: u32 = 8; // 1 << 14

const PACKET_LEN: u32 = 4096; // Only 190 needed but it must be equal to alignment below, x86_64 requires 4KB+
...
pub fn fill_frames_for_kernel(device: &mut DeviceQueue) {
    assert!(!device.needs_wakeup());

    let mut writer = device.fill(PACKET_RING_SIZE); // Trasffer all umem frames ownership to kernel?
    for i in 0..PACKET_RING_SIZE {
        let offset = i as u64 * PACKET_LEN as u64; // Each frame is 4096 bytes; compute offset
        writer.insert_once(offset);
    }
    writer.commit();
}
