...
use aya_ebpf::{
    bindings::xdp_action::{self, XDP_ABORTED},
    macros::{map, xdp},
    maps::{Array, HashMap},
    programs::XdpContext,
};
...
#[map]
pub static XDP_ROUTER_GLOBAL: Array<u8> = Array::<u8>::with_max_entries(GLOBAL_MAP_SIZE, 0);

#[map]
static XDP_ROUTER_MIRRORS: HashMap<u16, u8> =
    HashMap::<u16, u8>::with_max_entries(MIRROR_MAP_SIZE, 0);

#[map]
static XDP_ROUTER_ROUTES: HashMap<HalfRoute, HalfRoute> =
    HashMap::<HalfRoute, HalfRoute>::with_max_entries(REDIR_MAP_SIZE, 0);

#[xdp]
fn poc_xdp_router(ctx: XdpContext) -> u32 {
    match process(&ctx) {
        Ok(ret) => ret,
        Err(e) => {
            let msg = match e {
...
fn process_udp(ctx: &XdpContext, offset: usize, ip: &mut Ipv4Hdr, eth: &mut EthHdr) -> XdpResult {
    ...
    unsafe {
        match XDP_ROUTER_ROUTES.get_ptr_mut(&key) {
            Some(found) => {
                let found = *found;
    ...
    match XDP_ROUTER_GLOBAL.get(GlobalRule::Policy as u32) {
...
fn test(ctx: &XdpContext, old_ip: [u8; 4], new_ip: [u8; 4], check: u16, check_expect: u16) {
    ...
    if check == check_expect {
        info!(
            ctx,
            "{:i} -> {:x} ? {:x}",
            u32::from_be(new_ip_be),
            u16::from_be(check),
            u16::from_be(check_expect)
        );
    } else {
        error!(
            ctx,
